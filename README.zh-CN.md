...
项目正从**分类 baseline** 演进为 **embedding 检索 + RAG** 系统——把每张照片转成向量，在向量数据库中匹配，再用 LLM 生成有事实依据（grounded）的企鹅介绍。**检索内核已跑通**（top-1 95.9%，见 exp3）；档案/RAG 层与对话式界面正在推进中。
...
### 当前状态与下一步
**已完成**——CNN 检索路线（exp3）：一个可用的 **FAISS 向量数据库**，**top-1 95.9%**，支持**增量注册**（新企鹅存入特征即可登记，无需重训）与**开放集拒识**（照片模糊/非正面 → 提示游客重拍）。这就是 `identify_penguin` 工具的现成内核。
