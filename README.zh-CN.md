...
项目正从**分类 baseline** 演进为 **embedding 检索 + RAG** 系统——把每张照片转成向量，在向量数据库中匹配，再用 LLM 生成有事实依据（grounded）的企鹅介绍。一次**数据泄露审计**（[§5](#5-数据泄露审计与跨场次评估)）发现原来的头号数字被**训练/测试之间共享的同场次连拍**抬高了。在按场次的交叉验证下（[§6](#6-交叉验证下的度量学习)），当前识别准确率为 **macro top-1 0.559**（ArcFace 度量学习），softmax 基线为 **0.390**。本文所有准确率均为 **macro**——每只企鹅等权重，与它有多少张照片无关。剩余工作是档案/RAG 层与对话式界面。
...
图 01–05 覆盖分类实验，由 `plot_experiments.py` 从各 run 的日志重新生成。图 **06–11** 覆盖泄露审计与交叉验证评估，由 `analysis/plot_figures.py` 从 JSON 结果文件重新生成，并内嵌在 [§5](#5-数据泄露审计与跨场次评估) 与 [§6](#6-交叉验证下的度量学习) 中。两个脚本都输出到 `figures/`。
...
4. **数据是约束性瓶颈——但要分成两件事看。**
   ...
   但模型侧**并没有到头**：在已经有 ≥4 个场次的鸟身上，只改训练目标（softmax → ArcFace）就把准确率从 0.481 推到 0.669。数据和方法是互补的两个抓手，不是二选一。
...
**结果**（`analysis/eval_cv.py`，macro，1743 张照片，35 只个体）：

| macro 指标 | softmax + basic | **ArcFace + strong** | 配对差值 |
|---|---|---|---|
...
基线的交叉验证结果 **0.390 复现了 §5 单次划分的 0.389**，说明交叉验证没有引入偏差，ArcFace 的提升是在一个被验证过的基线上量出来的。所有差值区间都不含 0。置信区间也从 ±0.05 收窄到 ±0.022，因为被评分的照片从 261 张变成 1743 张。
...
rank-1 默认"镜头前这只鸟一定已经登记过"。但全群 81 只里只有 **35 只**入库，所以实际使用中系统会不断被问到陌生个体，它必须**拒识**而不是从已有名单里挑一个最像的。未知个体用留一法模拟：把某只鸟自己的身份从底库里移除，它的查询照片与剩余原型的最高相似度就是冒充者分数。

| 工作点 | softmax + basic | **ArcFace + strong** |
|---|---|---|
...
DIR@FAR=x 的含义是：在"未登记的鸟只有 x 的比例会被错误接受"这个阈值下，已登记的鸟中**既叫对名字、置信度又够高**的比例。**闭集 rank-1 的 0.559，在要求 99% 拒绝未登记个体时掉到 0.229** —— 这个落差就是"必须会说我不认识"所付的代价，也是真正描述部署可用性的数字。ArcFace 在每个工作点上都把它翻了近一倍，说明度量学习改善的不只是排序，还有置信度的分离。

**尚未完成：** 用全部 1743 张照片训练的部署模型——对它而言 0.559 是**保守**估计，因为每一折只用 80% 的数据训练，而部署模型会用 100%。
...
│  ├─ plot_figures.py                #   由结果文件重绘图 06-11
...
└─ runs/
   ...
   ├─ cv_softmax_basic/fold0..4/     # §6 基线，每折一个模型
   ├─ cv_arcface_strong/fold0..4/    # §6 ArcFace + 强增强
...
# 汇总各折：macro rank-1/rank-5/mAP/CMC/开放集 + 配对 bootstrap 比较
