...
|---|---|---|---|
| 原型 top-1 | 0.390 [0.367, 0.411] | **0.559** [0.535, 0.583] | **+0.169** [+0.145, +0.193] |
| 原型 top-5 | 0.746 [0.724, 0.769] | **0.782** [0.759, 0.804] | +0.036 [+0.012, +0.058] |
...
**尚未完成：** mAP 与 CMC（re-ID 领域惯用的报告组合）、开放集 TAR@FAR，以及用全部 1743 张照片训练的部署模型——对它而言 0.559 是**保守**估计，因为每一折只用 80% 的数据训练，而部署模型会用 100%。
...
│  ├─ eval_cv.py                     #   §6 折间汇总 + 配对 bootstrap
...
# 汇总各折，输出 macro + 置信区间，并用配对 bootstrap 比较两者
